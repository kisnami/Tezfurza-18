...
<h2><center>Result Winexpert</center></h2>
...
$queer="select * from winexpert where completed='*' order by earned_mark desc limit 5;";
$truncate="truncate table result_table_winexpert;";
...
while ($i< 5) {
	# code...
	$que1="insert into result_table_winexpert values('',$temp$stud_individual_id[$i]$temp,$temp$name[$i]$temp,$temp$year[$i]$temp,$temp$department[$i]$temp,$temp$college[$i]$temp);";
	mysql_query($que1,$conn);
	$i++;
	...
	$college=array();
	$stud_group_id=array();
	$tableQuery="select * from result_table_winexpert;";
