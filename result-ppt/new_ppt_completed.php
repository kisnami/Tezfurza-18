...
$query="SELECT centerlised_table.`stud_group_id`,centerlised_table.`college` FROM `centerlised_table` left join `ppt`on (centerlised_table.`ppt`='*'&& `ppt`.`completed`='*')&&(centerlised_table.`stud_group_id`= `ppt`.`stud_group_id`) where ppt.`stud_group_id`is null &&  centerlised_table.`ppt`='*';";
    ...
    <center><h2>Event Completed</h2></center>
        <div id="content">
            <table class="table1">
                ...
                $query_1="select stud_group_id from ppt where completed='*';";
